...
*   **IPC-gap localization (E13, 2026-08-03):** 🔻 **Gap found in the `*_M` scratchpad path; `*_M` is a solved-unknown, not a wall.** Captured the live JIT buffer from a real-mining run, disassembled, and classified every `ldr`/`ldp` by base register + immediate-consumer rate: **scratchpad (x2) loads are 64.3% serial** (`ldr x2 → immediate consume`), while dataset (x1/x20) and NEON/AES paths are **already pipelined**. The 0.551→0.648 IPC gap to XMRig therefore lives in the **scratchpad memory-op emission** — exactly the **`*_M` memory-op scheduler region** AGENTS.md records as having **diverged and been reverted (mechanism not identified)**. That makes `*_M` a *solved-unknown*: RandomX scratchpad ops are `read → transform → write back` with the transform depending on the loaded value, so a naive reorder likely tripped an address-aliasing / PC-coupling / AES-state constraint that was never root-caused. **Ground truth:** a compiled `xmrig-dev` on this device gives **1-thread IPC 0.628** (perf, core 3) vs armrx 0.551 — the ~14% per-cycle gap is real. **But the real-pool like-for-like test (2026-08-03) shows the IPC gap is a *phantom real-world deficit*:** 8-worker non-isolated, armrx = **21.4 H/s** vs XMRig = **27.77 H/s** (~23% behind); armrx WITH isolcpus = ~28.4 H/s (== XMRig). So the gap is armrx's **non-isolated hugepage + worker-affinity handling**, not codegen. **E15** is the real lever (close that gap); **E14** (root-cause `*_M`) is de-prioritized. The `jit_compiler_a64_static.S` E12 edit is retained as a documented dead-end reference. See `docs/experiments/next-iteration-plan.md` (E13/E14/E15).
*   **Real-pool non-isolated gap (E15, 2026-08-03):** 🔻 **Real ~23% gap found, NOT codegen — the next concrete lever.** Like-for-like on the herominers pool (8 workers, no `isolcpus`, same silicon): armrx **21.4 H/s** vs XMRig **27.77 H/s**. armrx WITH `isolcpus` = **~28.4 H/s** (= XMRig), so the deficit is armrx's default non-isolated behavior: (a) **hugepages** — XMRig auto-acquires them ("huge pages 100% 8/8"); armrx's E9 showed `MAP_HUGETLB` fails without root-reserved pages, falling back to **4 KiB**; (b) **affinity** — non-isolated armrx lets the OS scatter workers onto weak cores 4-7 (documented ~half-throughput penalty) while XMRig rides the fast cluster. Also: armrx takes ~150s to reach steady state vs XMRig's immediate full speed — same root (no hugepages + poor initial affinity). Fix = make armrx's *default* match XMRig: `madvise(MADV_HUGEPAGE)` fallback on the cache mapping + prefer fast-cluster cores 0-3 when `isolcpus` absent. A real, shippable ~20% win for the common (non-isolcpus) case. (libuv is unrelated to hashing throughput — it is XMRig's event loop, not the RandomX JIT — though it may explain XMRig's cleaner/faster startup; not a lead to chase.) See `docs/experiments/next-iteration-plan.md` (E15).
