...
*   **Superscalar generator timing-model A53 re-tune (2026-08-07):** ✅ **ADOPTED (Design C).** `src/superscalar.cpp` modeled x86's 2-MUL execution ports; the Cortex-A53 has a single integer-multiply port. Set `dependent_=true` on the MUL op in `IMULH_R`/`ISMULH_R` decompositions (2 lines) so the generator's cost model emits a program that spreads multiply contention. **Result:** `other_interlock_stall` **halved** (23.3→11.63M/hash) at **H/s parity** — on-device `bench_armrx --full-hash-only` 8w: main 13.48 vs adopted 13.98 H/s; real-pool `--dataset-mb=512` steady **~31.5 H/s** (no regression vs pre-adopt 31.5 H/s). The stall reduction is a free code-quality win (less heat, more headroom); 8w throughput is bounded by the two-cluster interconnect (silicon), not multiplies. Adopted as `91f5b2f`. Designs A + B (full model rewrite) broke reference hashes and were reverted. See `docs/briefs/2026-08-07-superscalar-timing-model.md`.
