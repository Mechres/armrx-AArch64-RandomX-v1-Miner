...
*   **Cross-hash boundary pipelining (Track D2):** ⚠️ **Measured null at E2E (2026-08-10).** Overlaps AES finalization of hash N with AES fill of hash N+1 via interleaved read/write. The microbench (`docs/experiments/t11-d2-microbenchmark.md`) showed −2.77% *AES-time* (IPC 1.735→1.787), but the **real E2E A/B on Lenovo @765MHz (light/8w, 200s) shows NO H/s benefit**: interleaved **27.01** vs sequential **27.25 H/s** (+0.9% favoring sequential, within noise). The AES-time win does not reach end-to-end throughput on the in-order A53. Default is still interleaved (not a regression), but the lever should be considered **closed-neutral** — it adds code complexity for zero hashrate gain. A compile guard `ARMRX_DISABLE_D2_PIPELINE` exists for the A/B; flip the default to sequential if the complexity isn't wanted.
