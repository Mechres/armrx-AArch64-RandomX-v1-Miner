...
*   **Real-pool non-isolated gap (E15, 2026-08-03):** 🔻 **~18-23% gap found, but BOTH leading hypotheses ruled out.** Like-for-like on the herominers pool (8 workers, no `isolcpus`, same silicon): armrx **21-23 H/s** vs XMRig **27.77 H/s**; armrx WITH `isolcpus` = **~28.4 H/s** (= XMRig). Originally hypothesized as hugepages + affinity. Both ruled out: (1) **hugepages** — as root armrx DOES get 128×2 MiB via `MAP_HUGETLB` (proven `HugePages_Free` 256→128) yet only reaches 22.9 H/s (+1.5) — not the lever; armrx's `virtual_memory.c` already uses XMRig's exact technique; (2) **affinity** — default `AffinityMode::All` (mining_engine.cpp:168) pins workers 1:1 to `core_order_`; on this no-cpufreq device that degenerates to 0..7 (coincidentally correct fast/weak split), so pinning is already right. **Cause remains open**: likely the real-program hash-loop efficiency vs XMRig, or 8-worker threading/contention overhead — NOT yet measured. Earlier "2× per-core" claim retracted (4-worker run read un-settled). No code change for E15; needs a clean settled per-core A/B first. See `docs/experiments/next-iteration-plan.md` (E15).
