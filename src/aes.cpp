...
[[nodiscard]] constexpr std::uint8_t rotate_left(std::uint8_t input, unsigned count) {
    return static_cast<std::uint8_t>((input << count) | (input >> (8U - count)));
}

[[nodiscard]] constexpr std::uint8_t gf_multiply(std::uint8_t left, std::uint8_t right) {
    std::uint8_t result{};
    for (unsigned bit = 0; bit < 8; ++bit) {
        if ((right & 1U) != 0U) result ^= left;
        const bool high_bit = (left & 0x80U) != 0U;
        left = static_cast<std::uint8_t>(left << 1U);
        if (high_bit) left ^= 0x1bU;
...
[[nodiscard]] constexpr std::uint8_t gf_inverse(std::uint8_t input) {
    if (input == 0U) return 0;
    std::uint8_t result = 1;
    std::uint8_t base = input;
    unsigned exponent = 254;
    while (exponent != 0U) {
        if ((exponent & 1U) != 0U) result = gf_multiply(result, base);
...
[[nodiscard]] constexpr std::uint8_t sbox(std::uint8_t input) {
    ...
    return static_cast<std::uint8_t>(inverse ^ rotate_left(inverse, 1) ^ rotate_left(inverse, 2) ^
                                     rotate_left(inverse, 3) ^ rotate_left(inverse, 4) ^ 0x63U);
}

[[nodiscard]] constexpr std::uint8_t inverse_sbox(std::uint8_t input) {
    const auto affine_inverse = static_cast<std::uint8_t>(
        rotate_left(input, 1) ^ rotate_left(input, 3) ^ rotate_left(input, 6) ^ 0x05U);
...
[[nodiscard]] AesBlock encrypt_transform(const AesBlock& input) {
    ...
    for (unsigned row = 0; row < 4; ++row) {
        for (unsigned column = 0; column < 4; ++column) {
            output[row + 4U * column] = byte(sbox(value(input[row + 4U * ((column + row) % 4U)])));
        }
    }
    for (unsigned column = 0; column < 4; ++column) {
...
[[nodiscard]] AesBlock decrypt_transform(const AesBlock& input) {
    ...
    for (unsigned column = 0; column < 4; ++column) {
        const auto base = 4U * column;
        const auto a0 = value(input[base]);
        const auto a1 = value(input[base + 1U]);
        const auto a2 = value(input[base + 2U]);
        const auto a3 = value(input[base + 3U]);
        mixed[base] = byte(gf_multiply(a0, 14) ^ gf_multiply(a1, 11) ^ gf_multiply(a2, 13) ^ gf_multiply(a3, 9));
        mixed[base + 1U] = byte(gf_multiply(a0, 9) ^ gf_multiply(a1, 14) ^ gf_multiply(a2, 11) ^ gf_multiply(a3, 13));
        mixed[base + 2U] = byte(gf_multiply(a0, 13) ^ gf_multiply(a1, 9) ^ gf_multiply(a2, 14) ^ gf_multiply(a3, 11));
        mixed[base + 3U] = byte(gf_multiply(a0, 11) ^ gf_multiply(a1, 13) ^ gf_multiply(a2, 9) ^ gf_multiply(a3, 14));
    }
    AesBlock output{};
    for (unsigned row = 0; row < 4; ++row) {
        for (unsigned column = 0; column < 4; ++column) {
            const auto source = (column + 4U - row) % 4U;
            output[row + 4U * column] = byte(inverse_sbox(value(mixed[row + 4U * source])));
        }
    }
    return output;
}
...
AesBlock aes_encrypt_round(const AesBlock& state, const AesBlock& round_key) {
    ...
    for (unsigned i = 0; i < output.size(); ++i) output[i] ^= round_key[i];
    return output;
}

AesBlock aes_decrypt_round(const AesBlock& state, const AesBlock& round_key) {
    AesBlock keyed{};
    for (unsigned i = 0; i < keyed.size(); ++i) keyed[i] = state[i] ^ round_key[i];
    return decrypt_transform(keyed);
}
