...
*   [ ] Conservative 2-3-instruction emitter lookahead scheduler — **implemented and correctness-verified 2026-07-25**, performance not yet measured. Attacks the IPC-0.708 stall budget directly, targeting item 14's measured `IMUL_R`/`IMULH_R`/`ISMULH_R`/`IMUL_RCP` dominant cost. A `src==dst` shared-physical-scratch-register hazard (invisible to the register-level hazard model) was found by a new 450-pair differential stress test (`tests/test_jit_scheduler_stress.cpp`) and fixed via bisection; full existing suite also green on-device. Next: `perf stat`/hashrate measurement to decide keep-vs-revert. See `PLAN.md` Phase 6 item 12.
