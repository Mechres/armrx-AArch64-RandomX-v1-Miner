...
- **Track C Phase A** (light-mode dataset-item register preservation) — **DONE 2026-07-29: WORKING.** Reduced `rx_calc_dataset_item` prologue from 112-byte/7-pair frame (x0-x13) to 80-byte/5-pair (x4-x13 only) — the prior attempt's exact same change, which previously hung on first JIT hash. Root cause of the hang never conclusively identified, but this time all tests pass cleanly including the diagnostic data-flow check (2500 seed/item pairs), KATs, JIT equivalence (16/16), determinism, and full mining suite. Expected 3-8% hashrate win (instruction-count axis), not yet quantified.
