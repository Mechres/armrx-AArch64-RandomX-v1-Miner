...
*   [ ] Self-directed instruction-count reconciliation for the remaining ~55% of the superscalar/dataset-derivation region — **static counting done, `perf record` self-profiling done (2026-07-24), points at JIT code not C++.** Live `perf record -g` on `./armrx --mine` (486K samples, verified via `/proc/<pid>/maps` that unresolved samples land inside real per-worker RWX JIT buffers) shows only ~14-15% of self-time in named C++ symbols — corrects the earlier hypothesis that the missing ~60M instructions/hash live in C++ (AES/Blake2b are separately measured at 0.3%/0.0% of hash time, too small). The remainder is JIT-generated code beyond what's already statically counted. Next: correlate perf's raw sample addresses against item 13's `JitDumpEntry` offset tables for real opcode-level attribution inside the JIT buffer — not yet built. No XMRig comparison, per `PLAN.md`'s clean-room boundary note. See `PLAN.md` Phase 6 item 14.
