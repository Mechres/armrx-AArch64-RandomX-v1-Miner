...
- **Peephole JIT coalescing is closed on evidence** (2026-07-25) — the main VM program region has a ~2.2× IPC penalty from memory-op stalls, not an instruction-count problem. Performance work is closed out for now; see `docs/plans/performance-plan-20260725.md` (gated steps) and `docs/plans/experimental-performance-ideas-20260725.md` (speculative backlog) if resuming.
