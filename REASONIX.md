...
- **Experimental flags** — `ARMRX_PGO`, `ARMRX_ENABLE_NEON_AES` (all default OFF, measured not beneficial on current hardware). `ARMRX_ENABLE_JIT_FAST_DIV_SQRT` was **removed 2026-08-10** (Newton-Raphson FDIV/FSQRT): a latent wrong-hash bug when enabled — it clobbered v0–v3 at runtime, corrupting the IMUL_RCP literal pool that `emitMovImmediate` reads via smov/umov — and a measured −1.1% regression. Reconstruct with v16–v31 (outside the v0–v15 pool) if ever re-attempted.
- **Peephole JIT coalescing is closed on evidence** (2026-07-25) — the main VM program region has a ~2.2× IPC penalty from memory-op stalls, not an instruction-count problem. See `docs/archived/plans/performance-plan-20260725.md` (gated steps) and `docs/archived/plans/experimental-performance-ideas-20260725.md` (speculative backlog) if code-level performance work resumes.
