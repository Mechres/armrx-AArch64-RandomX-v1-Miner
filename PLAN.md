...
*   **Side finding while doing this work — real build regression, documented (not root-caused):** the *standard documented* build command (`cmake -S . -B build -DARMRX_ENABLE_NATIVE=ON && cmake --build build -j`, no LTO workaround) fails on-device — the `armrx` executable hits the GCC15+musl+LTO `vsnprintf`/`always_inline` link error, caused by the §1.3 `main.cpp` split changing how LTO partitions that target. `bench_armrx` and the other test targets are unaffected. `README.md` and `REASONIX.md` updated to document `-DARMRX_DISABLE_LTO=ON` as currently required on this toolchain; root-causing why the split specifically triggers this is separate follow-up work, not done here.
