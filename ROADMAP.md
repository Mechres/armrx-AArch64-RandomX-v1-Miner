...
- **W3 — superscalar immediate-materialization density (closed, regression)** (2026-08-01) — the only evidence-backed W2-3 lever: replace C7-C9 MOVZ/MOVN+MOVK+ALU (~3 A64) with a single `LDR` from a per-original-index literal pool. Implemented scheduler-safely; correctness gate **passed** (equivalence 16/16, 2-way 40,020 derivations, determinism). **Perf A/B: −16% to −20% H/s** despite −20% instructions (cache-misses tripled 109M→339M, IPC 0.71→0.56): ~714 scattered literal loads/program thrash the cache on this in-order-ish A53. Fourth confirmation that fewer instructions ≠ faster here. Reverted bit-clean; tree at `c5ac985`. The W2-3 census holds — lever real but not realizable via literal-pool loads on this HW. Do not re-attempt. See `docs/experiments/w3-immediate-density.md`
