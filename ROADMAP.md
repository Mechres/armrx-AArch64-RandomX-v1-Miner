...
### In Progress
(none — all W1–W3 diagnostic/measurement backlog items closed on evidence. W3-2 now has its bisection instrument (`ARMRX_MAX_SWAPS` swap-budget hook + `test_scheduler_bisect`, device-verified) but the address-hoisting optimization itself stays blocked pending the actual memory-op-divergence bisection. W3-1 (Track C) is shipped/zero-impact; W3-3 (no-ABI trampoline) argued-against by the census. Next items need the JIT-compiler files, which are gated to comments-only until lifted)
