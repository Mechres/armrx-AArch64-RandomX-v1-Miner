...
- **Track C Phase A** (light-mode dataset-item register preservation) — **DONE 2026-07-29: MEASURED — ZERO IMPACT.** Reduced `rx_calc_dataset_item` prologue from 112-byte/7-pair frame (x0-x13) to 80-byte/5-pair (x4-x13 only). All tests pass cleanly including KATs, JIT equivalence, determinism. 8-worker interleaved benchmark: Phase A 25.06 H/s vs baseline 25.08 H/s — no measurable improvement. Root cause: caller-frame saving (4 instructions/call = 65,536 per hash) is 0.05% of ~134M instructions/hash. Code stays (correct, zero cost), but the next target must be the callee frame.

> **Correction (2026-08-01):** the Phase A change was re-applied on 2026-07-30 (c7c1b5c) and
> **hung the full-VM JIT** (100% CPU spin, zero hashes) — identical to the 2026-07-27 hang —
> then reverted (27e7c41). "Code stays" is false; the tree does not contain it. See
