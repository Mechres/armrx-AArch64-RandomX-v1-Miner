# armrx — Master Plan Archive: Phase 6 (Completed)

> **Archived 2026-07-25.** This document is the completed-work half of `PLAN.md`'s Phase 6
> section, split out once every item in it was done (all but one — `--rt-priority`, blocked on
> user device access — and the still-gated peephole-JIT idea), so the live plan could stay
> focused on genuinely open work. Nothing here is still open except where explicitly marked
> blocked/gated; it's kept verbatim as the historical record of *why* each change was made, not
> just *that* it was made. For the chronological, dated version of the same history, see
> `changelogs.md`; for the short-list actionable view at each point in time, see
> `NEXT_STEPS.md`; for Phases 1-5's equivalent archive, see
> `docs/archived/plan_completed_phases_1-5.md`.

---

## Phase 6 (2026-07-24 through 2026-07-25, completed): dual master-plan synthesis, on-device verification first
...
15. **Re-run `devbox_pgo_build` after any medium/long-term item lands meaningfully — re-checked 2026-07-25 after both scheduler commits (item 12), still a null.** First unpinned comparison (PGO vs. non-PGO `bench_armrx --full-hash-only`) showed a misleading ~2x gap (4.47 vs. 2.24 H/s) — turned out to be entirely a core-cluster artifact (caught by the user): this device's two 4-core clusters run at different clock speeds, and unpinned `bench_armrx` can land on either. Same non-PGO binary alone: 4.48 H/s on core 0, 2.24 H/s on core 4 -- a 2x swing with zero code difference. Re-measured properly (`taskset -c 0`, same core, both binaries): PGO 223570.61 μs/4.47 H/s vs. non-PGO 223203.66 μs/4.48 H/s — identical within noise. **Confirms the original null, on the current (post-scheduler) code shape.** Build reverted to normal (non-PGO) config. **Methodology note**: any future `bench_armrx` wall-clock comparison on this device MUST pin to a specific core (`taskset -c N`) — this is the same lesson item 12's `perf stat` A/B trials already learned the hard way, now confirmed to apply to plain wall-clock benchmarking too, not just `perf stat`.
...
18. **Three independent code reviews of the emitter scheduler (Deepseek, Gemini, Hermes) — cross-verified and acted on, 2026-07-25.** Requested specifically because item 12's scheduler is consensus-critical (silent-wrong-hash risk, not a crash risk) and had just gone through a non-trivial correctness arc (two empirically-found hazards). All three independently confirm no constructible failure scenario in the shipped code — CBRANCH anchor logic, the superscalar `IMUL_RCP` exclusion, index bounds, and cross-scheduler independence all check out across all three. One real, convergent finding: two of three (Deepseek, Hermes) independently caught that the `src==dst` doc comment was factually wrong about `h_IROL_R` (claimed it uses the shared x20 scratch register when `src==dst`; the code does the opposite — x20 only when `src!=dst`). Gemini restated the original (wrong) claim without checking it against the handler's actual code — a real quality gap in that review, not just noise (the user chose not to use Gemini for future scheduler-adjacent reviews as a result). Fixed: rewrote the doc comment to drop the falsified "x20 race" theory and record what's actually settled (the exclusion's *necessity* was already proven by the original bisection — a real divergence reproduced with it disabled, for a genuine `src==dst` case, `R=ISUB_R(dst==src==7)`) vs. still open (the exact mechanism). Also added, per Hermes's review: a fail-safe assert in `resolveInstructionType()` (previously silently collapsed any future unrecognized JIT handler to a NOP footprint — no hazard bits — which could let the scheduler swap something unsafely across it; now asserts loudly and fails safe, treating it as a barrier), and a comment documenting why `generateSuperscalarHash()`'s `num32bitLiterals=64` pin is load-bearing for scheduler safety. Verified 5/5 on-device after the changes. Full reports in `docs/audits/{emitter-scheduler-review,jit_scheduler_code_review_gemini,scheduler-review-2026-07-25}.md`. Committed `c92a1a9`.

    **A side attempt to resolve the `src==dst` mechanism definitively — abandoned, not shipped.** Tried building a hand-crafted-`Program` differential test (bypassing blake2b generation, comparing JIT vs. interpreter state after a single `run()` call) to force the excluded swap and observe directly whether it diverges. Hit an unrelated methodological trap instead: comparing VM state after one `run()` call (rather than the full 8-round `randomx_calculate_hash()` chain) shows spurious divergence for any program that never touches float registers — plausibly `run_jit()`'s `eMask`-into-`f[0]` side-channel trick leaving stale data when nothing overwrites it, though this wasn't confirmed before the attempt was abandoned as out of scope. Reverted the test-only `VirtualMachine` hook and the test file entirely rather than ship half-understood infrastructure. The exclusion's necessity remains proven by the original bisection; only its exact mechanism stays unresolved — noted as a real, if minor, open question for a future differential-harness attempt, done more carefully.
19. **Full codebase audit (Deepseek) — reviewed and verified, 2026-07-25.** User supplied `PROJECT_AUDIT_REPORT_20260725_Deepseek.md` (initially kept at repo root; moved into [`docs/audits/`](../audits/PROJECT_AUDIT_REPORT_20260725_Deepseek.md) later the same day alongside the other audit reports). Verified the audit's concrete, checkable claims against actual code/live device state rather than accepting them:
    - *§1.1, scratchpad huge-page residency "unverified"* — **refuted**, verified live on-device: `/proc/<pid>/smaps` merges adjacent same-protection anonymous VMAs into one entry, so the 2 MiB scratchpad doesn't appear as a separate line item — it's merged with the 256 MiB Argon2 cache into one 258 MiB (264192 kB) region showing `AnonHugePages=264192`, i.e. 100% huge-page-backed for the combined region. Not a real gap; already covered by item 1's original finding.
    - *§1.2 (`register_usage_[8] = {-1}` initializer) and §1.3 (rounding-mode sentinel)* — accurate but not new: both are already flagged with inline code comments explaining why they're safe (`// initializer is moot`, `// invalidate cache so CFROUND always calls fesetround`). Traced §1.3's specific claim through `reset_rounding_mode()` and confirmed it's a non-issue (the function sets the cache to 0 *and* actually calls `fesetround(FE_TONEAREST)` in the same call, keeping cache and hardware state in sync).
    - *§4.1 (CBRANCH with an unwritten target register wraps `pc` to 0 via `int16_t(-1)` → `++pc`)* — **confirmed accurate** by tracing `h_CBRANCH` → `execute_bytecode()`'s `pc = ibc.target` → `for (int pc = 0; pc < 256; ++pc)`. Correctly characterized by the audit as theoretical (RandomX's program generator spec-guarantees registers are written before being branched on) — no known real-world trigger, defensive `ARMRX_ASSERT` not yet added, low priority.
    - *§3.2 (`-frounding-math` missing from `CMakeLists.txt`)* — **confirmed real** via direct grep; the audit's own risk assessment (JIT emits raw AArch64 FP instructions, interpreter uses runtime bytecode dispatch, so the compiler can't constant-fold VM float values) holds up. Cheap, defensible, not yet applied — the one genuinely actionable item from this audit.
    - §2.1's cited scheduler IPC numbers (+0.233%/−0.036%) matched this project's own actual measurements exactly — a good signal the audit is grounded in real project docs, not fabricated.
    **No new bugs found; one previously-open question (§1.1) closed as a non-issue; one small, optional hardening item (`-frounding-math`) left for a future pass.**
