...
- **Fleet:** Lenovo (primary, fixed 765 MHz), Redmi 7A/pine (DVFS, ~1708 MHz,
  ...
  3.86 GiB RAM — **fleet re-validation (CORRECTNESS) DONE 2026-08-10**: native Termux
  build + correctness KATs PASS on Cortex-A55. **Hashrate baselined 2026-08-10** (real
  `pool-test` run, not the short bench window): light/all/8w = **33.61 H/s** (the ~21.84
  H/s @ 8w figure was a known short-window artifact, NOT thermal — device fan-cooled +
  board exposed); `fast/big-only/4w` = **41.89 H/s** (conservative floor; warm tail ~106).
  `--affinity-mode` is irrelevant at equal worker count (worker count is the only lever);
  full results in `docs/briefs/2026-08-10-unisoc-handoff.md` + `devices/unisoc-sc9863a-termux.md`.
  Fast-mode run COMPLETE — only fast-mode-capable fleet device).
- **⚠️ REVIEW STATUS: NOT CLOSED.** The three high-severity findings (🔴 hybrid, 🔴 DAG,
  FDIV/FSQRT) are resolved and merged, but this review has explicit OPEN items remaining
  (see §0.1). "Fully closed" is FALSE — do not treat the audit as done.
...
### 0.1 Open items remaining (review NOT closed)

The three high-severity findings are resolved+merged, but the following are explicitly
**OPEN** per this review's own §1/§5/§6 and the Deepseek/Luna tables. None are "closed."

| # | Open item | Source | Status |
|---|-----------|--------|--------|
| 1 | **LTO cross-plugin A/B** (T8-1) — acquire/build cross toolchain with working AArch64 LTO plugin, run A/B | Luna/Deepseek | ✅ resolved 2026-08-10 — modern toolchain (GCC 16.1.0, `aarch64-linux-musl-gcc-ar`/`gcc-ranlib` wrappers) DOES support cross LTO; objects carry `.gnu.lto_*` bytecode, links clean via gcc-ar (old AUR plain-`ar` hang no longer applies). A/B on Lenovo @ fixed 765MHz, light/8w, 200s: **no-LTO 27.20 H/s vs LTO 27.14 H/s = −0.2% (noise)**. LTO = measured NULL. Keep `ARMRX_DISABLE_LTO=ON` (no gain, adds link complexity + musl fortify quirk) |
| 2 | **AES generator scalar XOR disassembly** (T8-4) — one-time `objdump` of AArch64 binary; close if compiler already vectorized | Luna #1 | ✅ resolved 2026-08-10 — disassembled aarch64 binary: 63 `aese` + 117 aese/aesmc vector ops; `aes.hpp:350-369` XORs the 16-byte `AesBlock` via `veorq_u8` (`uint8x16_t`) = single 128-bit `eor v.16b`, NOT a scalar loop. Compiler already vectorized; no opt to make |
| 3 | **Unisoc real H/s** — re-measure `--affinity-mode=big-only --pool-test --seconds=200`; 21.84 is an artifact | T8-3 / handoff | ✅ resolved 2026-08-10 (light/all/8w = **33.61**; 21.84 was short-window artifact) |
...
Do NOT declare this audit "done" until at least #1–#6 are dispositioned or explicitly
deferred with rationale. Re-engaging external audits (Deepseek/Luna) later is still
valuable — prior "finished" declarations were wrong twice.
...
**RESOLVED by DELETION (2026-08-10):** the `ARMRX_ENABLE_JIT_FAST_DIV_SQRT` NR path was a latent wrong-hash bug (clobbered v0–v3 at runtime, corrupting the IMUL_RCP literal pool) and a measured −1.1% regression, frozen since 2026-07-21. Removed the NR handlers + CMake option (branch `try/t8-fdiv-remove`); `h_FDIV_M`/`h_FSQRT_R` now emit native FDIV/FSQRT only. On-device gate passed (test_jit_equivalence 16/16 byte-identical, armrx_tests reference hashes match) — byte-identical to prior shipping build since the NR path was dead code under the OFF flag. Latent bug eliminated; no live behavior change.
...
1. **Startup JIT autotune** ⭐ (top pick) — 🟡 **OPEN — needs more device tests.** Premise measured on current fleet only: native Bionic A/B on Unisoc **A55** (the one device with a different pipeline from A53), light/8w, 200s each (160s post-warmup) → **PAD-ON 33.87 H/s vs PAD-OFF (`ARMRX_NO_E24_PAD`) 33.78 H/s = −0.3%, tie, within noise**. A53 devices (Lenovo, Redmi) already E24-optimal; A55 a tie → **no win observed in current 3-device fleet**. NOT closed: other AArch64 uarchs (A55 revs, A72/A76, etc.) untested — pad width may still matter there. **Crowdsource-it: `docs/open-experiments/e24-pad-width.md`** (reproducible build+run steps, report template). Scheduler-window sub-lever excluded (consensus-constrained, T8-2). Machinery not built yet; revisit if a device shows a pad-width delta. Evidence on `try/jit-autotune` (empty branch; measured-and-parked).
...
| FDIV/FSQRT v0–v3 clobber | Deepseek (latent) | `jit_compiler_a64.cpp:2074` (FDIV v0–v2, comment "safe — not used by other handlers"), `:2118` (FSQRT v0–v3); flag `ARMRX_ENABLE_JIT_FAST_DIV_SQRT` OFF by default (`CMakeLists.txt:35`) | **RESOLVED by DELETION 2026-08-10.** Confirmed latent wrong-hash bug: the NR path clobbers v0–v3 at runtime, corrupting the IMUL_RCP literal pool (`emitMovImmediate` reads v0–v15 via smov/umov at `:1456-1467`; independently traced in `docs/archived/audits/independent-audit_2026-08-07.md:35`). Flag was OFF (shipping-safe) but a trap; measured −1.1% regression (`perf-tracking.md:167`), frozen since 2026-07-21. **Removed** the NR handlers + `ARMRX_ENABLE_JIT_FAST_DIV_SQRT` CMake option (branch `try/t8-fdiv-remove`); handlers now emit native FDIV/FSQRT only. On-device gate: `test_jit_equivalence` 16/16 byte-identical, `armrx_tests` Input1/Input2 hashes match reference — byte-identical to prior shipping build (NR path was dead code under OFF flag). No live behavior change; latent bug eliminated. |
