...
DAG = **+10.3% cycles, +11.9% instr, −14.3% H/s** (M1≈M2, reproducible). Adopt gate (≥0.5% cycles improvement in both M runs) → FAIL. **Root cause (CORRECTED 2026-08-07, `changelogs.md:418,429`):** the regression is **NOT** a longer emitted sequence — emitted length is **order-invariant** under the DAG scheduler. The −14.3% H/s came from **planner runtime overhead**: the O(n²) hazard-matrix build plus ~9 heap allocations *per hash* (the planner runs at compile time per program, not in the hot loop, but its `sys` time tripled). So the failing dimension was planner complexity/allocations, not instruction count. **Code kept gated** behind `ARMRX_DAG_SCHED=1` (default OFF) as a documented negative result — not enabled, not deleted. **Closure label (corrected 2026-08-10):** "JIT scheduler reordering is exhaustively closed" overstates the evidence — the DAG *concept* was never isolated from its implementation cost. A *cheap* planner (bitmap hazard matrix, schedule reuse) is a legitimate, documented-reopening variant; expected ROI is small (E20's 0.73% legal-move rate). The current implementation is correctly closed; the *family* is not exhaustively disproven. See `changelogs.md` + `ROADMAP.md`. Next real lever = worker-cluster placement (Idea 2), not JIT emission.
