...
**🔴 (b) DAG scheduler closure overreaches + stale wrong root cause.**
`docs/briefs/2026-08-06-dag-scheduler-attempt.md:13` and `ROADMAP.md` still state "reorder forces a longer emitted sequence" — but the 2026-08-07 correction (`changelogs.md:418,429`) proved emitted length is **order-invariant**; the −14.3% was **planner runtime** (O(n²) hazard matrix + ~9 heap allocs/hash, sys time tripled). "JIT scheduler reordering is exhaustively closed" overstates the evidence. A cheap planner (bitmap hazard matrix, schedule reuse) is a legitimate, documented-reopening variant (small expected ROI — E20's 0.73% legal-move rate). **Action:** correct the root cause in the brief + ROADMAP and downgrade the ledger label to "this implementation failed."
...
|-------|--------|---------------------------|--------|
| Hybrid `--dataset-mb>0` wrong on-device | Deepseek 🔴(a) | `README.md:167-171` ("✅ correct & recommended, verified same-nonce") vs `tests/test_mining.cpp:438-448` ("NOT producing correct end-to-end hashes on-device") | **CONFIRMED contradiction.** Both at HEAD. Recommended config affected. Needs on-device same-nonce KAT to settle direction — do not edit claims until then. |
