...
| L2 | ~~Region-scoped armrx-vs-XMRig generated-code comparison~~ **Reframed 2026-07-24 (clean-room boundary, `PLAN.md` Phase 6 item 14):** self-directed instruction-count reconciliation of armrx's own generated code — no XMRig-internals inspection | armrx's own `--jit-dump` instrumentation + `.S` source counting (`jit_compiler_a64.cpp`, `jit_compiler_a64_static.S`) | Reconcile the remaining ~55% instructions/hash; only then judge a peephole pass | Do NOT start the 3–6-week peephole effort on the old aggregate numbers; XMRig disassembly is permanently out of scope |
