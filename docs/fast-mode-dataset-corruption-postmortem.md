...
### GCC 15 + musl + LTO also broke the `armrx` executable link, unrelated to any of the above — since root-caused and fixed

At the time this postmortem was first written, on-device `armrx` failed to link with LTO enabled, and was initially (incorrectly) suspected to be caused by the `main.cpp` → `cli_parser.cpp`/`miner_app.cpp` split (PLAN.md §1.3). That theory did not hold up: building the pre-split commit reproduces the identical failure, and forcing single-partition LTO (`-flto-partition=one`) doesn't fix it either, ruling out WHOPR partitioning as the mechanism.

The actual cause: Alpine's `fortify-headers` package wraps libc calls (`vsnprintf`, reached here via `std::to_string(double)` → libstdc++'s `__to_xstring`) in `extern`+`always_inline` functions, which GCC's LTO cannot always resolve — it hard-errors ("function body can be overwritten at link time") instead of emitting an out-of-line call. This is independent of any code in this repository; it has been latent since the on-device `gcc 15.2.0-r6 → r8` package upgrade on 2026-07-13, well before this session, and just hadn't been hit by a from-scratch LTO build of the `armrx` target specifically until now.

**Fixed** in `CMakeLists.txt`: scope `-U_FORTIFY_SOURCE -D_FORTIFY_SOURCE=0` to just the `armrx` target when LTO is enabled, rather than disabling LTO project-wide via `ARMRX_DISABLE_LTO`. Verified on-device: the standard documented build command (LTO on, no workaround flag) now links `armrx` cleanly and passes `ctest` 7/7.
