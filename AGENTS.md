...
- **No cpufreq on device (verified 2026-08-01)**: MSM8929 + postmarketOS kernel has `CONFIG_CPUFREQ_DT=y` but the device tree carries NO CPU OPP table (`operating-points-v2` absent) → no policies, no `scaling_governor` under `/sys/docs/devices/system/cpu/cpu*/`. CPU runs at a fixed firmware-set ~765 MHz (perf-derived; not thermal — idle 36°C, load 48-50°C). ~1.1 GHz NOT achievable without DT/kernel change + reflash. Don't suggest governor/`cpupower` fixes; treat 765 MHz as the real clock for all rate math.
